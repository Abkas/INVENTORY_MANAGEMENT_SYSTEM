...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['sales_date'], $_POST['quantity'], $_POST['total_price'], $_POST['customer_id'], $_POST['product_id'])) {
    ...
    if ($sales_date !== '' && $quantity > 0 && $total_price !== '' && $customer_id > 0 && $product_id > 0) {
        mysqli_query($conn, "INSERT INTO sales (sales_date, quantity, total_price, customer_id, product_id, user_id) VALUES ('$sales_date', '$quantity', '$total_price', '$customer_id', '$product_id', '$user_id')");
...
// Fetch sales
$sales_result = mysqli_query($conn, "SELECT s.*, c.customer_name, p.product_name FROM sales s JOIN customer c ON s.customer_id = c.customer_id JOIN product p ON s.product_id = p.product_id ORDER BY s.sales_id DESC");
